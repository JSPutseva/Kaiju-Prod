...
def test_qc_cannot_request_transfer_before_level_3(
    ...
    assert "REQUEST_ADJACENT_TRANSFER" in response.json()["detail"]
    assert "level 1" in response.json()["detail"]
...
def test_cd_cannot_request_adjacent_transfer_at_level_4(
    ...
    assert response.status_code == 403
    assert "REQUEST_ADJACENT_TRANSFER" in response.json()["detail"]
    assert "level 4" in response.json()["detail"]
...
def test_qc_cannot_organize_transit_at_level_3(
    ...
    assert response.status_code == 403
    assert "ORGANIZE_TRANSIT" in response.json()["detail"]
